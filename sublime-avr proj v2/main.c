...
// Компилятор хочет, чтобы аргументом _delay_us была константа.
// Поэтому приходится делать велокостыль.
// кроме того, максимальная задержка в мкс = 768/частота чипа в мгц
{
...
int main(void)
{
	...
	while (1)
	{
		if (pedal())
		{
			...
			// по нажатию педали отработать заданное количество импульсов
			for (unsigned char counter = 0; counter < PULSECOUNT; counter++){
			...
			while(pedal()); // ничего не делать, пока педаль нажата
		}
